 ...
 * provider.
 *
 * <p>Findings carry a file path, line range, generic per-rule text, and the offending line as a bounded
 * snippet (already secret-masked; the secret rule additionally redacts the matched value) so a reviewer
 * can see the exact code without opening the file.
 *
        ...
        for (int i = 0; i < lines.length; i++) {
            ...
            if (line.contains("TODO") || line.contains("FIXME")) {
                out.add(new Finding(path, lineNo, lineNo, ReviewFindingSeverity.LOW, "MAINTAINABILITY",
                        ...
                        "todo-fixme", BigDecimal.valueOf(0.95), snippetOf(line)));
            }
            if (jsTs && line.contains("console.log")) {
                out.add(new Finding(path, lineNo, lineNo, ReviewFindingSeverity.LOW, "DEBUG_CODE",
                        ...
                        "console-log", BigDecimal.valueOf(0.90), snippetOf(line)));
            }
            if (java && line.contains("System.out.println")) {
                out.add(new Finding(path, lineNo, lineNo, ReviewFindingSeverity.LOW, "DEBUG_CODE",
                        ...
                        "system-out-println", BigDecimal.valueOf(0.90), snippetOf(line)));
            }
            if (java && line.contains("printStackTrace")) {
                out.add(new Finding(path, lineNo, lineNo, ReviewFindingSeverity.MEDIUM, "ERROR_HANDLING",
                        ...
                        "print-stack-trace", BigDecimal.valueOf(0.90), snippetOf(line)));
            }
            addLeftoverSecret(path, line, lineNo, out);
        }

        addEmptyCatch(path, content, lines, out);
    }
    ...
    private void addLeftoverSecret(String path, String line, int lineNo, List<Finding> out) {
        ...
        while (m.find()) {
            ...
            if (!m.group(1).contains(REDACTED)) {
                // Show WHERE the secret is, but redact the value so the snippet never exposes it.
                String redactedLine = line.replace(m.group(1), REDACTED);
                out.add(new Finding(path, lineNo, lineNo, ReviewFindingSeverity.HIGH, "SECURITY",
                        ...
                        "hardcoded-secret", BigDecimal.valueOf(0.60), snippetOf(redactedLine)));
                return; // one finding per line is enough
            }
        }
    }

    private void addEmptyCatch(String path, String content, String[] lines, List<Finding> out) {
        Matcher m = EMPTY_CATCH.matcher(content);
        while (m.find()) {
            int lineNo = lineNumberOf(content, m.start());
            String line = lineNo >= 1 && lineNo <= lines.length ? lines[lineNo - 1] : "";
            out.add(new Finding(path, lineNo, lineNo, ReviewFindingSeverity.MEDIUM, "ERROR_HANDLING",
                    "Empty catch block",
                    ...
                    "empty-catch", BigDecimal.valueOf(0.70), snippetOf(line)));
        }
    }

    /** Trim and cap the offending line so each finding carries a readable, bounded snippet. */
    public static String snippetOf(String line) {
